...
from .new_bot import airbnb_support_bot
import json
...
def whatsapp_webhook(request):
    ...
    if request.method == "POST":
        ...
        if msg.get("type") == "text":
            ...
            history = WhatsAppMessageSerializer(WhatsAppMessage.objects.filter(wa_id=sender_id), many=True).data
            available_properties = client.get_listings()
            
            
            reply = airbnb_support_bot(history, available_properties, text_body, datetime.now())
            reply = reply.replace("python", '')
            reply= reply.replace("```", '')
            reply = json.loads(reply) 
            start = reply.get("check_in_date", None)
            end = reply.get("check_out_date", None)
            property_name = reply.get("property_name", None)
            
            
            if start and end and property_name:
                availability = client.check_availability(property_name, start, end)
                reply = airbnb_support_bot(history, available_properties, text_body, datetime.now(), availability=availability)
                reply = reply.replace("python", '')
                reply= reply.replace("```", '')
                reply = json.loads(reply) 
                start = reply.get("check_in_date", None)
            ...
            response = reply.get("response", None)
            print(reply)
            ...
            whatsapp_message_object = WhatsAppMessage.objects.create(
                wa_id        = sender_id,
            ...
            try:
                result = send_whatsapp_message(sender_id, response)
